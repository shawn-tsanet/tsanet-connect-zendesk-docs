...
The integration is complete at the end of Step 5. If you also want an optional, externally-hosted SLA breach alert inside Zendesk, see the separate [GitHub Actions SLA Monitor (Optional)](https://github.com/tsanetgit/Zendesk_App/blob/main/GitHub_Actions_SLA_Monitor.md) document — it needs its own GitHub repository, GitHub Actions, and stored secrets, so it's kept out of this guide.
...
The connection from Step 4 does nothing by itself — the flows that create and update Zendesk tickets live in the **flow bundle** under [`zis/`](https://github.com/tsanetgit/Zendesk_App/tree/main/zis/). Three calls deploy it (full detail, per-instance substitutions, and gotchas in [`zis/README.md`](https://github.com/tsanetgit/Zendesk_App/blob/main/zis/README.md)):

1. **Upload** [`zis/tsanet_connect_bundle.json`](https://github.com/tsanetgit/Zendesk_App/blob/main/zis/tsanet_connect_bundle.json) (after substituting your custom field IDs) to `POST /api/services/zis/registry/tsanet_connect/bundles`
...
The bundle can also forward an agent's **public reply** to the partner as a TSANet note (issue #34) — so the partner sees agent replies automatically. **Internal** comments are never forwarded; only public replies reach the partner. It needs a second inbound webhook (`source_system: zendesk`, `event_type: public_comment`) plus a Zendesk webhook + trigger. Full setup is in [`zis/README.md` → *Inbound comment forwarding*](https://github.com/tsanetgit/Zendesk_App/blob/main/zis/README.md).

**You are finished here.** The core integration (ZIS connection + flow bundle, Steps 1–5) is complete. For an optional externally-hosted SLA breach alert, see [GitHub Actions SLA Monitor (Optional)](https://github.com/tsanetgit/Zendesk_App/blob/main/GitHub_Actions_SLA_Monitor.md).
